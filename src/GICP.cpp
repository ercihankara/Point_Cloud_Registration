...
int main(int argc, char** argv){
    ...
    double gicp_alignment_error = 0.0;
    double sum_gicp_alignment_error = 0.0;
    double avrg_gicp_alignment_error = 0.0;
    double gicp_convergence_time = 0.0;
    ...
    std::ofstream trajectory_file("/home/ercihan/catkin_ws/src/point_cloud/trajectory_scenario_2_gicp.txt");
    ...
    ros::Time gicp_start_time = ros::Time::now();
    for(const rosbag::MessageInstance& msg : view){
        ...
        if(msg.getTopic() == lidar_topic){
            ...
            // set GICP parameters if needed
            gicp.setMaximumIterations(10); // reduce
            //gicp.setTransformationEpsilon(1e-8);
            gicp.setMaxCorrespondenceDistance(0.1); // increase
            ...
            *cloud_target += *cloud_aligned;
            ...
            // calculate alignment error using RMSE for GICP
            ...
            for(size_t i = 0; i < cloud_aligned->points.size(); ++i){
                ...
                gicp_alignment_error += dx * dx + dy * dy + dz * dz;
            ...
            gicp_alignment_error = sqrt(gicp_alignment_error / valid_correspondences);
            sum_gicp_alignment_error += gicp_alignment_error;
    ...
    ros::Time gicp_end_time = ros::Time::now();
    gicp_convergence_time = (gicp_end_time - gicp_start_time).toSec();

    // save the aligned point cloud
    pcl::io::savePCDFileASCII("/home/ercihan/catkin_ws/src/point_cloud/point_cloud_scenario_2_gicp.pcd", *cloud_aligned);

    std::cout << "processing data is done!" << endl;

    // print average alignment error for GICP
    avrg_gicp_alignment_error = sum_gicp_alignment_error/error_count;
    ROS_INFO("GICP Average Alignment Error: %f", avrg_gicp_alignment_error);
    // print convergence time for ICP
    ROS_INFO("GICP Convergence Time: %f seconds", gicp_convergence_time);
