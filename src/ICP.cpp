...
int main(int argc, char** argv){
    ...
    double icp_alignment_error = 0.0;
    double sum_icp_alignment_error = 0.0;
    double avrg_icp_alignment_error = 0.0;
    double icp_convergence_time = 0.0;
    ...
    std::ofstream trajectory_file("/home/ercihan/catkin_ws/src/point_cloud/trajectory_scenario_2_icp.txt");
    ...
    for(const rosbag::MessageInstance& msg : view){
        ...
        if(msg.getTopic() == lidar_topic){
            ...
            *cloud_target += *cloud_aligned;
            ...
            // calculate alignment error using RMSE for ICP
            ...
            for(size_t i = 0; i < cloud_aligned->points.size(); ++i){
                ...
                icp_alignment_error += dx * dx + dy * dy + dz * dz;
            ...
            icp_alignment_error = sqrt(icp_alignment_error / valid_correspondences);
            sum_icp_alignment_error += icp_alignment_error;
    ...
    ros::Time icp_end_time = ros::Time::now();
    icp_convergence_time = (icp_end_time - icp_start_time).toSec();

    // save the aligned point cloud
    pcl::io::savePCDFileASCII("/home/ercihan/catkin_ws/src/point_cloud/point_cloud_scenario_2_icp.pcd", *cloud_aligned);

    std::cout << "processing data is done!" << endl;

    // print average alignment error for ICP
    avrg_icp_alignment_error = sum_icp_alignment_error/error_count;
    ROS_INFO("ICP Average Alignment Error: %f", avrg_icp_alignment_error);
    // print convergence time for ICP
    ROS_INFO("ICP Convergence Time: %f seconds", icp_convergence_time);
